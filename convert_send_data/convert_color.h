...
String convert_color(uint16_t* data) {
  //Serial.println(data);
  long color;
  String color_string = "";
  color = expandColor(pgm_read_word(data));
  //color = pgm_read_word(&(data));
  color_string += (color >> 16) & 0xff;
  color_string += ' ';
  color_string += (color >> 8) & 0xff;
  color_string += ' ';
  color_string += color & 0xff;
  return color_string;
}
